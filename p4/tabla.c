 ...
 * Nota: para los parámetros se añaden después
 *   como variable así que esto sirve.
 */
  ...
  for (int j = tope - 1; j >= 0; j--) {
    if (!strcmp(TS[j].nombre, identificador) &&
       (TS[j].tipo_entrada == variable ||
         TS[j].tipo_entrada == procedimiento)) {
...
int esDuplicado(char * identificador) {
  for (int j = tope - 1; j >= 0; j--) {
    if (!strcmp(TS[j].nombre, identificador) &&
      ...
      // Si llegamos al final del bloque
      return 0;
    }
  }
}
...
/*
 * Sal de bloque y elimina de la tabla de símbolos
 * todos los símbolos hasta la última marca
 */
void salBloqueTS(){
  if(DEBUG){
